...
class MoneyActivity : AppCompatActivity() {
    ...
    private fun countMoney(todayDay: Int) {
        ...
        val moneyMonth = (moneyOfDay * countDayMonth) + 21000   // Количество денег в месяц вместе с квартплатой
        textViewMoneyAMonth.text = "Денег в месяц: $moneyMonth"
    }

    private fun saveMoneyPref(money: Int, checkBox: Boolean) {
    ...
    override fun onStop() {
        super.onStop()
        saveMoneyPref(editTextMoney.text.toString().toInt(), checkBox.isChecked)
    }
